    ...
    public function addReviews(): void
    {
        Product::all()
            ->each(function (Product $product) {
                for ($i = 0; $i < 25; $i++) {
                    $product->createReview(Review::factory()->setProduct($product)->make()->toArray());
                }
            });

        Review::all()->each(function (Review $review) {
            $helps = [];
            $totalHelps = random_int(10, 20);
            $reactionType = fake()->randomElement(['like', 'dislike']);

            for ($i = 0; $i <= $totalHelps; $i++) {
                $helps[] = [
                    'type' => $reactionType,
                    'owner_id' => User::query()->inRandomOrder()->first()->getKey(),
                    'owner_type' => (new User())->getMorphClass(),
                ];
            }

            if ($reactionType === 'like') {
                $review->likes()->createMany($helps);
            } else{
                $review->dislikes()->createMany($helps);
            }

            $review->media()->createMany(
                Media::factory()->count(3)->make()->toArray(),
            );
        });

        Review::query()->limit(10)
            ->get()
